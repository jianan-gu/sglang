"""
Unit tests for the Intel CPU AMX implementation of FlashMLA's sparse FP8
decode path: ``flash_mla_with_kvcache_cpu``.

The kernel and its Python wrapper were added in commit 3f8c38d on branch
``jianan/dsv4_ref``.  These tests exercise the wrapper across:

* page sizes (KV-cache block sizes): 64 and 128
* FP8 KV-cache layouts (V32_FP8Sparse and MODEL1_FP8Sparse), which differ in
  ``d_qk`` (576 vs. 512), ``d_v`` (512 vs. 448), tile size, and per-block
  storage layout
* index dtypes: ``torch.int32`` and ``torch.int64``
* presence/absence of optional inputs:
    - ``attn_sink``      (per-head bias added in log-sum-exp space)
    - ``topk_length``    (per-batch valid top-k length)
    - ``extra_k_cache``  / ``extra_indices_in_kvcache`` / ``extra_topk_length``

For every configuration the kernel output is compared against a small
in-test PyTorch reference that mirrors
``sglang.srt.flashmla_tests.ref.ref_sparse_attn_decode``.  Both code paths
go through the same FP8 dequantization helper
(``flashmla_quant.dequantize_k_cache``) so quantization error is excluded
from the comparison and tolerances can be tight.

Usage:
    python3 -m unittest test_flash_mla_with_kvcache_cpu
"""
