...
class TestMiniMaxH3PackedSequence(unittest.TestCase):
    ...
    def test_fl2va_first_last_cond_blocks_use_exact_rope_span(self):
        ...
        self.assertEqual(
            built["input_ids"][text_len : text_len + cond_rows].unique().tolist(),
            [MINIMAX_H3_IMGVID_COND_ID],
        )
        ...
        video_pos = built["img_pos"][cond_rows:]
        self.assertEqual(int(built["input_ids"][video_pos[0]]), MINIMAX_H3_VIDEO_FIRST_ID)
        self.assertEqual(int(built["input_ids"][video_pos[-1]]), MINIMAX_H3_VIDEO_LAST_ID)
        self.assertTrue(built["image_mask"][cond_pos.reshape(-1)].all())
    ...
    def test_fl2va_keyframe_index_validation_is_defensive(self):
        ...
        for frame_indices in (
            ...
            with (
                ...
                self.assertRaisesRegex(
                    ValueError, r"requires (?:integer )?keyframe_frame_indices"
                ),
            ):
                minimax_h3_packed_sequence(
                    **common,
        ...
        with self.assertRaisesRegex(ValueError, "frame_count is required"):
            minimax_h3_packed_sequence(
