...
_ALLOWED_CONDITION_KEYS = frozenset({"type", "uri", "role", "frame_index"})
    ...
    *,
    ...
    for index, cond in enumerate(conditions):
        ...
        if role not in (
            ...
            raise ValueError(
                f"{cpath}.role must be keyframe or reference, "
                f"got {role!r}"
            )
    ...
    **_extra_kwargs: Any,
    ...
    if normalized_target.get("duration_seconds") is not None:
        requested_frame_count = int(
            round(float(normalized_target["duration_seconds"]) * MINIMAX_H3_SUPPORTED_FPS)
        )
