...
def _resolve_deferred_temporal_shape(
    ...
    facts = probe_facts[int(sources[0].condition_index)]
    try:
        duration_seconds = float(facts["audio_duration_seconds"])
    except (KeyError, TypeError, ValueError) as exc:
    ...
    frame_count = planner.align_frame_count(int(round(duration_seconds * fps)))
    ...
    shape.update(
        {
            ...
            "video_latent_t": planner.video_latent_t(frame_count),
            "audio_latent_t": planner.audio_latent_t(aligned_duration),
        }
    )
