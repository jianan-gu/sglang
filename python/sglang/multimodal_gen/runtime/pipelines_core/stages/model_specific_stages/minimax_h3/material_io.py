...
MINIMAX_H3_MATERIAL_VALIDATED_EXTRA_KEY = "minimax_h3_material_validated"
...
def minimax_h3_cleanup_temp_dirs(
    ...
    if owners is None or "material" in selected:
        ...
        batch.extra.pop(MINIMAX_H3_MATERIAL_VALIDATED_EXTRA_KEY, None)
...
def _base64_decoded_size(uri: str, payload_start: int) -> int:
    encoded_size = 0
    padding = 0
    saw_padding = False
    ...
    *,
    ...
    if condition_type == "image":
        ...
        if width <= 0 or height <= 0:
            raise ValueError("MiniMax H3 image material has no positive display geometry")
        return {
    ...
    try:
        result = subprocess.run(
            [
                ...
                "-show_entries",
                "stream=codec_type,width,height,duration,sample_rate,channels,avg_frame_rate,r_frame_rate,nb_frames,nb_read_frames,sample_aspect_ratio,display_aspect_ratio:stream_tags=rotate:stream_side_data=rotation:format=format_name,duration",
                "-count_frames",
                "-of",
                "json",
                "-i",
    ...
    for stream in video_streams:
        try:
            ...
            raise ValueError("MiniMax H3 video material has invalid dimensions") from exc
        if width <= 0 or height <= 0:
    ...
    if primary_video is not None:
        ...
        raw_count = primary_video.get("nb_read_frames") or primary_video.get(
            "nb_frames"
        )
        try:
...
def _validate_material_once(
    ...
    *,
    ...
    validated = batch.extra.setdefault(MINIMAX_H3_MATERIAL_VALIDATED_EXTRA_KEY, set())
    probe_facts = batch.extra.setdefault(MINIMAX_H3_MATERIAL_PROBE_EXTRA_KEY, {})
    key = (uri, condition_type)
    cached = probe_facts.get(key)
    if key in validated and isinstance(cached, dict):
        return cached
...
def _stream_base64_material(
    ...
    *,
    ...
    decoded_size = _base64_decoded_size(uri, payload_start)
