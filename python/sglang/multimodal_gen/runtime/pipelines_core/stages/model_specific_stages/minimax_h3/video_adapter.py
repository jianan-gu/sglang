...
class MiniMaxH3VideoModelAdapter(BaseVideoModelAdapter):
    ...
    def _reject_retired_cfg_fields(kwargs: dict[str, Any]) -> None:
        ...
        if guidance_scale is not None:
            raise ValueError(
                "guidance_scale is not supported: MiniMax H3 serves only the "
                "CFG-distilled single-positive-branch checkpoint"
            )
        guidance_scale_2 = kwargs.pop("guidance_scale_2", None)
        if guidance_scale_2 is not None:
            raise ValueError(
                "guidance_scale_2 is not supported: MiniMax H3 serves only the "
                "CFG-distilled single-positive-branch checkpoint"
            )
        negative_prompt = kwargs.pop("negative_prompt", None)
        if negative_prompt is not None:
            raise ValueError(
                "negative_prompt is not supported: MiniMax H3 serves only the "
                "CFG-distilled single-positive-branch checkpoint"
            )
    ...
    def lower_sampling_params(
        ...
        kwargs = self._common_sampling_kwargs(request_id, request)
    ...
    def validate_transport_options(
        ...
        *,
        ...
        super().validate_transport_options(request, model_path=model_path)
        for field_name in ("audio_flow_shift", "audio_guidance_scale"):
            self._positive_finite_extra(request, field_name)
    ...
    def expand_for_dispatch(
        ...
        *,
        ...
        if int(getattr(batch, "num_outputs_per_prompt", 1)) > 1 or isinstance(
            getattr(batch, "seed", None), list
        ):
            expansion_kwargs = {}
            if num_prompts != 1 or prompt_index != 0:
                expansion_kwargs = {
                    "num_prompts": num_prompts,
                    "prompt_index": prompt_index,
                }
            expanded = expand_request_outputs(batch, **expansion_kwargs)
            if len(expanded) > 1:
                import msgspec
                ...
                for output_index, output in enumerate(expanded):
                    output.extra[MINIMAX_H3_TEMP_DIRS_EXTRA_KEY] = {}
                    resolved_plan = output.extra.get("minimax_h3_resolved_plan")
                    if resolved_plan is None:
                        continue
                    selected_seed = output.seed
                    if isinstance(selected_seed, bool) or not isinstance(
                        selected_seed, int
                    ):
                        raise ValueError(
                            f"expanded MiniMax H3 output {output_index} must "
                            "carry an integer seed to refresh its resolved "
                            f"plan, got {selected_seed!r}"
                        )
                    output.extra["minimax_h3_resolved_plan"] = msgspec.structs.replace(
                        resolved_plan,
                        seed=selected_seed,
                    )
            return expanded
    ...
    async def validate_final_outputs(
        ...
        return await asyncio.to_thread(
            self.validate_final_outputs_sync,
            output_paths,
            batch,
        )
...
def _probe_minimax_h3_output_fields(
    ...
    *,
    ...
    try:
        probe = subprocess.run(
            [
                ...
                "-count_frames",
                "-show_entries",
                "stream=codec_type,codec_name,pix_fmt,width,height,avg_frame_rate,nb_frames,nb_read_frames,duration,sample_rate,channels:format=format_name,duration",
                "-of",
                "json",
                str(path),
            ],
        ...
        raise RuntimeError(f"ffprobe failed for final MiniMax H3 output{suffix}") from exc

    try:
        payload = json.loads(probe.stdout)
    except (TypeError, ValueError) as exc:
        raise RuntimeError("ffprobe returned invalid JSON for MiniMax H3 output") from exc
    ...
    if width <= 0 or height <= 0:
        raise RuntimeError(f"generated MiniMax H3 MP4 has invalid size {width}x{height}")
    ...
    if frames_raw not in {None, "", "N/A"} and "/" in rate_raw:
        try:
            if fps > 0:
                duration = int(frames_raw) / fps
        except (TypeError, ValueError, ZeroDivisionError):
            pass
    if duration <= 0:
        try:
...
register_video_model_adapter(MiniMaxH3PipelineConfig, MiniMaxH3VideoModelAdapter)
