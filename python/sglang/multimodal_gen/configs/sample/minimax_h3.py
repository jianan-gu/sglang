...
class MiniMaxH3SamplingParams(SamplingParams):
    ...
    def _validate(self) -> None:
        ...
        if (
            ...
            and self.seed + self.num_outputs_per_prompt - 1 > _MINIMAX_H3_MAX_SIGNED_SEED
        ):
            raise ValueError(
    ...
    def build_request_extra(self, *, _seed_override: int | None = None) -> dict:
        ...
        if self.task is not None:
            ...
            extra["minimax_h3_canonical_request"] = minimax_h3_validate_canonical_request(
                task=self.task,
                prompt=self.prompt,
    ...
    def refresh_request_extra_after_output_expansion(self, req: Any) -> None:
        """Rebuild canonical identity from the selected scalar Req seed."""

        selected_seed = getattr(req, "seed", None)
        if isinstance(selected_seed, int):
            req.extra.update(self.build_request_extra(_seed_override=selected_seed))
