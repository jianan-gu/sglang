...
import os
MMMU_INPUT_PROMPT_LEN = int(os.getenv("MMMU_INPUT_PROMPT_LEN", "0"))
import aiohttp
import numpy as np
import requests
...
def sample_mmmu_requests(
    ...
    if len(mmmu_dataset) > num_requests:
        if random_sample:
            ...
            print("MMMU: Choose data samples ids:", indices)
    ...
    for i, example in enumerate(sample_dataset):
        try:
            ...
            if image is not None:
                ...
                check_token_ids = tokenizer.encode(question)
                check_question_len = len(check_token_ids)

                if MMMU_INPUT_PROMPT_LEN != 0:
                    if MMMU_INPUT_PROMPT_LEN > check_question_len:
                        pad_len = MMMU_INPUT_PROMPT_LEN // check_question_len
                        question = question * pad_len
                    else:
                        question = question[0:MMMU_INPUT_PROMPT_LEN]
                ...
                print("testing MMLU sample with total prompt size: ", prompt_len)
