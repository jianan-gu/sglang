    ...
    for (int i = begin; i < end; ++i) {
      ...
      if (!is_cross_attn) {
        ...
        for (int n = 0; n < num_keys; n += BLOCK_N) {
          ...
          for (int row = 0; row < m_size; ++row) {
            if (sliding_window_size > 0) {
              // Sliding-window mask for the extend (self) part, matching the Triton
              // reference (python/sglang/srt/layers/attention/triton_ops/
              // extend_attention.py): a key is kept only when
              //   query_pos <= key_pos + sliding_window_size,
              // i.e. keys older than the window (key_pos < query_pos -
              // sliding_window_size + 1) are masked. The window is one-directional:
              // future keys (key_pos > query_pos) are never masked here. For causal
              // attention they are already masked by the triangle above; for
              // bidirectional attention they are intentionally kept, exactly as the
              // Triton kernel does. Both query and key live in the extend segment,
              // so query_pos is (m + row) and key_pos is the column index.
              const int q_pos = m + row;
              // Columns [n, q_pos - sliding_window_size + 1) of this block are masked.
              int old_cnt = (q_pos - sliding_window_size + 1) - n;
              old_cnt = std::min(std::max(old_cnt, 0), n_size);
              // Skip the whole block (no softmax contribution) if every key is too old.
              if (old_cnt >= n_size) {
                continue;
              }
              if (old_cnt > 0) {
                fill_stub(s_i + row * BLOCK_N, -std::numeric_limits<float>::infinity(), old_cnt);
              }
            }
            flash_attn_softmax<scalar_t, BLOCK_M, BLOCK_N>::apply(
                s_i, s_delta, v_prime, s_prime, m_prime, m_size, n_size, padded_n_size, head_size_v, sm_scale, row);
