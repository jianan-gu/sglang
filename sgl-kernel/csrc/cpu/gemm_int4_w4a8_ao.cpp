...
at::Tensor da8w4_linear_impl(
    ...
    at::ScalarType output_dtype) {
      RECORD_FUNCTION(
        "sgl-kernel::da8w4_linear_impl", std::vector<c10::IValue>({input, weight}));
...
#define call__da8w4_linear_impl(cpublas_can_pack, sym_quant_act) \
  ...
  TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
    // activation
    m.def("da8w4_linear_cpu(Tensor input, Tensor input_scales,Tensor input_qzeros,Tensor weight,Tensor weight_scales,Tensor weight_qzeros,Tensor compensation, Tensor? bias, ScalarType output_dtype) -> Tensor");
    m.impl("da8w4_linear_cpu", torch::kCPU, &da8w4_linear_impl);
    m.def("da8w4_linear_prepack_cpu(Tensor weight,Tensor scales,Tensor qzeros) -> (Tensor, Tensor,Tensor,Tensor)");
    m.impl("da8w4_linear_prepack_cpu", torch::kCPU, &da8w4_linear_prepack_impl);
    // m.def("per_token_quant_int8_cpu(Tensor act) -> Tensor");
